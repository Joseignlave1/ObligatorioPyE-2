...
#1) tabla con frecuencias, probabilidades empíricas y distribuciones acumuladas empíricas
...
#2) esperanza y varianza empírica
...
plt.hist(cancelaciones, bins=range(min(cancelaciones), max(cancelaciones)+2), align='left', rwidth=0.8, color='skyblue', edgecolor='black')
...
lam = esperanza  # parámetro lambda
...
plt.figure(figsize=(8, 5))
plt.hist(cancelaciones, bins=range(min(cancelaciones), max(cancelaciones)+2), align='left', rwidth=0.8, color='lightgrey', edgecolor='black', label='Datos empíricos')
...
#6) probabilidades bajo modelo Poisson
